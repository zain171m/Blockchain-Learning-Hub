...
const researchPaperId = startButton.getAttribute("data-id");
const type = startButton.getAttribute("data-type");
...
function quizCreator() {
    ...
    for (let i of quizArray) {
        ...
        console.log(i)
        const options = [i.option1, i.option2, i.option3, i.option4];
        ...
        question_DIV.innerHTML = i.question_text;
        div.appendChild(question_DIV);
        //options
        div.innerHTML += `
    <button class="option-div" onclick="checker(this)">${i.option1}</button>
     <button class="option-div" onclick="checker(this)">${i.option2}</button>
      <button class="option-div" onclick="checker(this)">${i.option3}</button>
       <button class="option-div" onclick="checker(this)">${i.option4}</button>
    `;
...
startButton.addEventListener("click", () => {
    ...
    const csrftoken = getCookie('csrftoken'); // Implement a function to retrieve the CSRF token

   
    // Fetch quiz questions using AJAX with the research paper ID as a parameter
    fetch(`/get_quiz_questions/${researchPaperId}/`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRFToken': csrftoken, // Include the CSRF token in the 'X-CSRFToken' header
          },
        body: JSON.stringify({ type }),
      })
    .then(response => response.json())
    .then(data => {
        // Access quiz questions from the JSON response
        quizArray = data.quiz_questions;
        console.log(quizArray)
        initial(); // Initialize the quiz with the loaded questions
        })
...
// Function to retrieve the CSRF token from a cookie
